...
1st call:-
power(3,5):-The caller of this is main.
...
temp=power(3,1):-The caller of this function is power(3,2).
...
temp=power(3,0):-The caller of this function is power(3,1).
Now,in this call, the value of x is 3 and n is 0.so,we have reached our base case(n==0),so 1 will be stored to temp of power(3,0) because 1 is being returned from base case.
----------------------------------------------------------------------------------

Now,current function is power(3,0),we will do temp=temp*temp that is 1=1*1 and then we will check the value of n of our callers, the caller of power(3,0) is power(3,1) and 1 is an odd number so, we will return temp * x that is = 1*3=3 to the caller.

Now,the caller temp=power(3,1) have the value equals to 3 which is returned from power(3,0).

Now,current function is power(3,1),we will do the temp=temp*temp that is 3=3*3 which is equals to 9 and then we will check the value of n of our callers, the caller of power(3,1) is power(3,2) and 2 is an even number so, we will return temp that is = 9 to the caller.

Now,the caller temp=power(3,2) have the value equals to 9 which is returned from power(3,1).

Now,current function is power(3,2),we will do the temp=temp*temp that is 9=9*9 which is equals to 81 and then we will check the value of n of our callers, the caller of power(3,2) is power(3,5) and 5 is an odd number so, we will return temp*x that is = 81*3=243 to the caller.
